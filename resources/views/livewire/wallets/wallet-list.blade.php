...
<div class="p-4 lg:p-8 space-y-6">
    ...
    {{-- Wallet Form Modal --}}
    @if($showForm)
    <div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" x-data x-init="$el.querySelector('[data-modal]').focus()">
        <div class="absolute inset-0 bg-black/50 backdrop-blur-sm" wire:click="$set('showForm', false)"></div>
        <div data-modal tabindex="-1"
             class="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 space-y-4"
            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    @error('balance') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <button wire:click="save"
                    class="flex-1 py-2.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-white text-sm font-medium transition-colors">
                    Simpan
                </button>
            </div>
        </div>
